...
const AuthForm = () => {
  ...
  const authForms: Record<AuthProcessI, JSX.Element | (() => JSX.Element)> = {
    ...
    forgotPassword: <ForgotPassword setFormType={setFormType} setAuthError={setAuthError} />,
  };
  return (
    <Stack spacing={30}>
      {/* <pre>{JSON.stringify(userCred, null, 2)}</pre> */}
      <Heading1 color='#333333' style={{ textAlign: 'center' }}>{titles[formType]}</Heading1>
      
      <>{authForms[formType]}</>
      {authError && (
        <Subheading3 color="#FF0055">
          {authError}
        </Subheading3>
      )}
      
      <Subheading3 color='#a1a1a1' style={{ width: '87%', margin: '0 auto', textAlign: 'center' }}>
