...
type AuthProcessI = 'signup' | 'confirm' | 'login' | 'forgotPassword' | 'submitPassword';

const Login = () => {
  const { cognitoLogin, cognitoRegister, cognitoConfirmRegistration, cognitoForgotPassword, cognitoSubmitNewPassword } = useAuth();
  ...
  const submitPasswordHook = useForm<PasswordReset>({
    ...
    validate: {
      ...
      password: (value) => (
        value.length > 8 && /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/.test(value) ? null : (value.length < 8 ? 'Must be at least 8 characters' : 'Must contain uppercase, lowercase, number and special case characters')),
      confirm_password: (value, values) => (value.length > 8 && value === values.password ? null: (value.length > 8? 'Passwords did not match': 'Required')),
    }
  })
  // Form titles
  const titles: Record<AuthProcessI, [string, string]> = {
    ...
    login: ['Sign Into App', ''],
    forgotPassword: ['Reset Password', 'A verification code will be sent to your email.'],
  ...
  const signupForm = (
    <>
      <form onSubmit={signupHook.onSubmit(async (values) => {
  ...
  const confirmForm = (
    <form onSubmit={confirmHook.onSubmit(async (values) => {
      ...
      <Stack>
        <TextInput placeholder="Email" {...confirmHook.getInputProps('email')} />
        <TextInput placeholder="Please enter the code here" {...confirmHook.getInputProps('code')} />
        {authError && <Text color="red" fz={14}>{authError}</Text>}
        <Button color="dark" type="submit">
          Verify
        </Button>
  ...
  const loginForm = (
    <>
      <form onSubmit={loginHook.onSubmit(async (values) => {
        ...
        <Stack>
          <TextInput placeholder="Email" {...loginHook.getInputProps('email')} />
          <PasswordInput placeholder="Enter Password" {...loginHook.getInputProps('password')} />
          {authError && <Text color="red" fz={14}>{authError}</Text>}
          <Button color="dark" type="submit">
            Log In
          </Button>
  ...
  const forgotPasswordForm = (
    <form onSubmit={forgotPasswordHook.onSubmit(async (values) => {
      ...
      <Stack>
        <TextInput placeholder="Email" {...forgotPasswordHook.getInputProps('email')} />
        {authError && <Text color="red" fz={14}>{authError}</Text>}
        <Button color="dark" type="submit">
          Continue
        </Button>
  ...
  const submitPasswordForm = (
    <form onSubmit={submitPasswordHook.onSubmit(async (values) => {
      if(!userEmail) return;
      ...
      <Stack>
        <TextInput placeholder="Enter verification code" {...submitPasswordHook.getInputProps('code')} />
        <PasswordInput placeholder="Enter new password" {...submitPasswordHook.getInputProps('password')} />
        <PasswordInput placeholder="Confirm new password" {...submitPasswordHook.getInputProps('confirm_password')} />
        {authError && <Text color="red" fz={14}>{authError}</Text>}
        <Button color="dark" type="submit">
          Submit
        </Button>
